...
fn main() {
    ...
    // Cloning the partition data
    let xs_clone = xs.clone();
    let xs_clone0 = xs.clone();
    // Creating the 2 threads
    let t1 = thread::spawn(move || map_data(&xs_clone[0]));
    let t2 = thread::spawn(move || map_data(&xs_clone0[1]));
    let _r1 = t1.join().unwrap();
    let _r2 = t2.join().unwrap();
    
    // Appending the intermediate sums
    ...
    // Calling partition_data to partition the data into equal partitions
    let xs0 = partition_data(num_partitions, &v);
    
    // Calling print_partition_info to print info on the partitions that have been created
    print_partition_info(&xs0);
    
    let mut intermediate_sums0 : Vec<usize> = Vec::new();
    let mut t = vec![];

    // Creating one thread per partition and uses each thread to concurrently process one partition
    for x in 0..num_partitions {
        let xs0_clone = xs0.clone();
        t.push(thread::spawn(move || map_data(&xs0_clone[x])));
    }
    
    // Joining all of the threads
    for x in t {
        let _r = x.join().unwrap();
        intermediate_sums0.push(_r);    
    }

    // Print the vector with the intermediate sums
    println!("Intermediate sums = {:?}", intermediate_sums0);

    // REDUCE STEP: Process the intermediate result to produce the final result
    let sum0 = reduce_data(&intermediate_sums0);
    println!("Sum = {}", sum0);
}
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    ...
    let mut xs: Vec<Vec<usize>> = Vec::new();
    let size = v.len() / num_partitions; // Dividing the number of integers into equal size partitions
    let mut start = 0;
    let mut end = size;
    let mut count = v.len() % num_partitions; // Remainder

    // Looping through number of partitions
    for x in 0..num_partitions {
        let mut xs0 : Vec<usize> = Vec::new();
        
        // Pushing the elements into their respective partitions
        // For example for ./main 5 150 this for loop would basically illustate:
        // summation of 0-29, then summation of 30-59, summation of 60-89, summation of 90-119, summation of 120-149
        for i in start..end {
            xs0.push(v[i]);
        }
        ...
        // In case the number of elements (v.len()) is not a multiple of num_partitions, 
        // some partitions can have one more element than other partitions
        // This basically adds the remaining numbers. For example, for ./main 5 153 the math would be:
        // 150 + summation of 0-29, 151 + summation of 30-59, 152 + summation 60-89, summation of 90-119, summation of 120-149
        if count > 0 {
            xs[x].push(v.len() - count);
            count -= 1;
        }
        
        // Increasing the start and end values for the next summation calculation
        start += size;
        end += size;
    }
    
    // Returning the partitions that contain all elements that are in the input vector
    xs
}
